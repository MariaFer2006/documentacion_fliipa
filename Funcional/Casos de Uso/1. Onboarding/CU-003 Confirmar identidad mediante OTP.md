...
| **Flujo principal** | 1. El sistema envía un código OTP por WhatsApp.<br>2. El cliente ingresa el código recibido.<br>3. El sistema valida el código.<br>4. El sistema envía un segundo código OTP por correo electrónico.<br>5. El cliente ingresa el código recibido.<br>6. El sistema valida el código y permite continuar al siguiente paso del proceso. |
...
| **Historias de usuario relacionadas** | HU-004 (Confirmar identidad)<br>HU-005 (Reintentar o corregir el envío del OTP) |
| **Estado en plataforma** | Implementado (`send-otp.ts`, `validate-otp.ts`). **Hallazgos de seguridad vigentes**: existe un código comodín hardcodeado que valida cualquier OTP (RNF-001), y el canal SMS está simulado — no envía mensajes reales (RNF-007). Ambos deben priorizarse antes de producción. |
| **Referencias** | Fuente: fichas HU-004 y HU-005 — *Historias de Usuario — Fliipa*, carpeta "1. Onboarding" (repositorio `documentacion_fliipa`, María Fernanda Herazo). |
